...
#define NORMALIZE_PRESSURE_VALUES true
...
int LURD_release_pressures[4] = {INITIAL_LEFT_PRESSURE, INITIAL_UP_PRESSURE, INITIAL_RIGHT_PRESSURE, INITIAL_DOWN_PRESSURE};
...
int VDCM_vibrationPadding = 10;

int oldValueWeight = 2;
...
void setPressure(int LURD_index, int value)
{
  LURD_pressures[LURD_index] = value;
  LURD_release_pressures[LURD_index] = value * releaseMultiplier;
}
...
void updateAnalogValues() {
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    {
      if (LURD_state[i] == 1 && LURD_values[i] < LURD_release_pressures[i])
      {
