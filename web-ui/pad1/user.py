...
if (add_user):
    ...
    print "<a href=/pad1/pads.py?cur_user="+new_user+">Edit sensitivities</a>"
    print '''<script>setTimeout(function() { window.location = "pad1/pads.py?cur_user=%s" }, 1000) </script>''' % new_user
else:
    print "Error adding user. (Does this user already exist?)"
    print "<a href=/pad1/pads.py>"
    print '''<script>setTimeout(function() { window.location = "pad1/pads.py" }, 1000) </script>'''
print '''</body>'''
print '''</html>'''
